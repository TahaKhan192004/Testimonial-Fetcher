...
/**
 * 5 attempts per hour per IP by default. Uses Upstash when configured, otherwise a
 * Supabase table. Fails open on infrastructure errors so a rate limiter
 * outage never blocks a real submission.
 */
export async function checkRateLimit(ip: string, bucket = "submit", limit = DEFAULT_LIMIT): Promise<{ allowed: boolean }> {
